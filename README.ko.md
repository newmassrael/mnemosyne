...
- **atomic store** (`docs/.atomic/workspace.atomic.json`) 가 단일, 직접
  검증되는 source of truth — 타입드 레코드 (Section / ChangelogEntry /
  FrozenList / CrossRef) + append-only 감사 의미.
- 사람은 디자인 히스토리를 `mnemosyne-cli query` 로 읽는다. spec 내용의
  SSOT 는 커밋된 **EPUB** (normative_excerpt 텍스트는 그 EPUB의
  재검증된 투영). markdown-render 모델(옛 `GENERATED.md`)은 Round 400 에서
  제거됨. AI는 typed primitive 로 쓴다.
- 모든 mutation 은 typed primitive 통과 — 저장 전에 atomic 불변식
  (cross-ref orphan reject, append-only / frozen-ledger) 강제.
...
**상태:** Phase 0 hardening. 500+ 테스트 그린. Mnemosyne는 자기 자신을
dogfood — 자체 디자인 히스토리는 atomic store
(`docs/.atomic/workspace.atomic.json`) 에 저장되며, `mnemosyne-cli query`
로 읽는다.
...
| 크레이트 | 역할 |
|---|---|
| `mnemosyne-atomic` | atomic store (JSON SSOT) + mutate primitive |
| `mnemosyne-query` | store 위 읽기 투영 (query / report-*) |
| `mnemosyne-validate` | citation / coverage / drift 검증 |
| `mnemosyne-store` | RocksDB CF 레이아웃 (파생 인덱스) |
| `mnemosyne-core` | 타입드 fact 브리지 |
| `mnemosyne-cascade` | Salsa cascade query |
| `mnemosyne-server` | gRPC + 감사 append 표면 |
| `mnemosyne-cli` | 프로덕션 CLI (validate / mutate / query) |
...
```bash
mnemosyne-cli validate-workspace   # T1 orphan + atomic ledger + style
mnemosyne-cli validate-code-refs   # citation defense ([plugins.set_equality_validator] 설정 시)
```

이 명령들이 baseline을 뽑는다 — T1 orphan 총합, T3/T4 스타일 위반,
atomic ledger sync, 그리고 소스 안의 spec-id
...
atomic store JSON 이 단일 직접-검증 산출물이다:

```
typed mutate primitive ──► atomic store JSON  (단일 SSOT)
                                  │
                  mnemosyne-cli query / report-*  (읽기 투영)
```

전형적인 mutation 흐름:

1. 작성자나 AI 가 typed primitive 호출 (예: `set_section_intent`).
2. Primitive 가 쓰기 전에 atomic 불변식 (cross-ref orphan reject,
   append-only / frozen-ledger) 강제.
3. 통과 시 atomic store JSON 을 temp 파일 + atomic rename 으로 쓴다.
4. `validate-workspace` 가 이후 매 호출에서 store 불변식 (T1 orphan,
   citation hygiene, drift) 을 재검증.

markdown-render 모델(tera 템플릿 → `GENERATED.md`, `parse(emit) ==
typed_facts` round-trip 게이트)은 Round 400 에서 제거됨: store 가 단일
직접-검증 SSOT 이고, 사람은 `mnemosyne-cli query` 로 디자인 히스토리를
읽으며, spec 내용은 커밋된 EPUB 에 산다. 읽기 경로는 atomic store 에서
SectionView JSON 을 바로 반환.

CLI / MCP 서버 / pre-commit hook — 어디서 호출하든 동일한 코드 경로
(`mnemosyne-atomic` 의 typed mutate primitive + `mnemosyne-validate` 의
검증) 를 거친다. 한 구현, 세 진입 표면.
...
3 개의 hook 가 자동 실행된다:
- `pre-commit` — code-citation defense, workspace validate (atomic
  sidecar 가 staged 일 때), `cargo fmt --all --check` (`.rs` 가 staged
  일 때).
...
### 왜 atomic store 인가, raw markdown 이 아니라
...
- Mnemosyne `set_section_impact_scope(target=§40)`: validator 가 §40
  존재를 확인하고, store 안의 모든 cross_ref 를 원자적으로 갱신한다.
...
- [docs/SCHEMA_GUIDE.md](docs/SCHEMA_GUIDE.md) — `mnemosyne.toml` 모든
  필드 + preset.
- `mnemosyne-cli query` — Mnemosyne 자체 디자인 히스토리 (atomic store
  changelog); spec 내용은 커밋된 EPUB 에 산다.
