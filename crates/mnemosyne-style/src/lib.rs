...
fn default_ruleset_with_glossary(glossary: BTreeMap<String, BTreeSet<String>>) -> Vec<StyleRule> {
            ...
            for para in split_paragraphs(body) {
                if has_enumeration_pattern(para) {
                    out.push(make_violation(
                        ...
                        Some(
                            "convert `(i)/(ii)/(iii)` or `1./2./3.` enumerations to `- ` bullets"
                                .into(),
                        ),
                    ));
                }
...
mod tests {
    ...
    fn strong_carry_classifier_anchors() {
        ...
        assert!(is_strong_carry_section(
            "15/spec-mutate-api-surface-extension-123-ratify"
        ));
