...
use mnemosyne_style::{check_style, default_ruleset, StyleSeverity, StyleViolation};
...
fn collect_workspace_violations() -> Vec<StyleViolation> {
    ...
    for path in WORKSPACE_DOC_PATHS {
        ...
        let content =
            fs::read_to_string(&abs).unwrap_or_else(|e| panic!("read {}: {}", abs.display(), e));
    ...
    let atomic_store =
        AtomicStore::load(&AtomicStore::default_sidecar_path(&root)).expect("atomic sidecar load");
    let mut all = Vec::new();
...
fn classify_section(section_id: &str) -> SectionKind {
    ...
    let is_numeric_head = !head.is_empty() && head.chars().all(|c| c.is_ascii_digit() || c == '.');
    if is_numeric_head {
...
fn re_audit_severity_baseline_anchored() {
    ...
    assert_eq!(
        v.len(),
        40,
        "total violations drifted from Round 241 baseline"
    );
...
fn re_audit_per_doc_severity_anchored() {
    ...
    for x in &v {
        let sev: &'static str = if x.severity == StyleSeverity::Warn {
            "warn"
        } else {
            "info"
        };
...
fn re_audit_design_md_sd_top_level_grouping() {
    ...
    assert_eq!(
        total, 2,
        "DESIGN.md SD top-level total drifted from Round 241 baseline"
    );
}
...
fn re_audit_non_design_sd_breakdown_anchored() {
    ...
    assert_eq!(
        total, 2,
        "non-DESIGN SD total drifted from Round 241 baseline"
    );
}
...
fn re_audit_cleanup_budget_recalibrated() {
    ...
    assert_eq!(
        sd_design_top_region, 0,
        "Round 155 scope SD pool drifted (carry baseline 0)"
    );
    assert_eq!(
        sd_design_top2_region, 1,
        "Round 156 scope SD pool drifted (carry baseline 1)"
    );
    assert_eq!(
        sd_design_remaining, 1,
        "Round 157 scope SD pool drifted (Round 241 baseline 1)"
    );
    ...
    assert_eq!(
        non_design, 2,
        "Round 158 scope SD pool drifted (Round 241 baseline 2)"
    );
