...
use mnemosyne_style::{check_style, default_ruleset, StyleViolation};
...
fn audit_classification_baseline_anchored() {
    ...
    assert_eq!(
        v.len(),
        40,
        "total violations changed (Round 241 baseline 40)"
    );
    assert_eq!(
        by_cat
            .get(&AuditCategory::AcceptableCarry)
            .copied()
            .unwrap_or(0),
        31,
...
fn audit_per_rule_per_category_anchored() {
    ...
    let expected: &[(&str, AuditCategory, usize)] = &[
        ...
        ("max_paragraph_length", AuditCategory::SubstantivelyDirty, 4),
        ("max_section_body_length", AuditCategory::HardCase, 5),
        ("max_sentence_length", AuditCategory::SubstantivelyDirty, 0),
    ];
...
fn audit_decision_matrix_path_anchored() {
    ...
    assert!(
        !alpha_triggered,
        "α not triggered post-241 (ac={:.3} < 0.80)",
        ac
    );
