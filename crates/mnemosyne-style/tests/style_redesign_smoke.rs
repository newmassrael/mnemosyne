...
fn skip_top_level_numeric_strong_carry() {
    ...
    let para = v
        .iter()
        .filter(|x| x.rule_id == "max_paragraph_length")
        .count();
    let sent = v
        .iter()
        .filter(|x| x.rule_id == "max_sentence_length")
        .count();
    assert_eq!(
        para, 0,
        "max_paragraph_length must skip §43 (top_level_numeric)"
    );
    assert_eq!(
        sent, 0,
        "max_sentence_length must skip §43 (top_level_numeric)"
    );
}
