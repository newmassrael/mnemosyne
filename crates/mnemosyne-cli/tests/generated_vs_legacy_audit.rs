...
use mnemosyne_atomic::{append_changelog_entry, AtomicStore, ChangelogEntryDraft};
...
use mnemosyne_style::{check_style, default_ruleset_with_config, StyleSeverity};
...
fn load_workspace_section_ids() -> BTreeSet<String> {
    ...
    let atomic = AtomicStore::load(&AtomicStore::default_sidecar_path(&root)).unwrap_or_default();
