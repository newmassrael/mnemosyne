...
fn round_trip_seven_docs_mandatory_preserved() {
 ...
 println!();
 println!("=== Round 72 round-trip 7/7 carry validation (Round 70 emitter row 8 separation then) ===");
...
fn workspace_typed_facts_state_populates_bodies_and_line_anchors() {
 ...
 assert!(anchor_39 > 0, "§39 anchor must be 1-indexed (got {})", anchor_39);

 // §66 body + anchor — Bootstrap stages source.
 let body_66 = design
 .bodies
 .get("66")
 .expect("§66 body must populate");
 // §66 body = top-level paragraph between heading and first nested ###
 // (Bootstrap stages etc. nested subsections §66/... in separate entry).
 assert!(body_66.contains("Choice") || body_66.contains("design doc") || body_66.contains("Self-application"),
 ...
 assert!(anchor_66 > anchor_39, "§66 anchor (= {}) must be after §39 (= {})",
 anchor_66, anchor_39);
}
