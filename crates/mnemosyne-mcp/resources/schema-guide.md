...
| `[map]` | **the spatial-graph gate's config** (`validate-map`): `path` (a `map/v1` nodes+edges file), `place_kind` (which registered `entity_kinds` id is spatial — not hardcoded), `severity`, optional `sha256`. A map node must be a registered place entity and an edge endpoint a map node; without this the gate is off |
