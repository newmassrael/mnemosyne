...
pub fn cross_ref_orphan_reject(doc: &ParsedDoc) -> Vec<ValidationError> {
    let section_id_set: BTreeSet<&str> =
...
pub fn changelog_entry_append_only(prev: &ParsedDoc, curr: &ParsedDoc) -> Vec<ValidationError> {
...
pub fn frozen_list_membership_delta(prev: &ParsedDoc, curr: &ParsedDoc) -> Vec<ValidationError> {
