...
/// [`AtomicSection`] so reviewers can verify code citations without fetching the
/// upstream HTML. A spec-facing WRAPPER (R759, P3c) around the one provenance
/// substrate [`ContentExcerpt`]: the shared `excerpt` carries the content-SSOT
/// anchor + verbatim text + drift hash (the same substrate narrative
/// `content_excerpt` / `scene_cast` use), and the two spec-only fields record the
/// UPSTREAM origin the baseline was captured from. The content-SSOT anchor
/// (`excerpt.anchor`, an EPUB CFI once a baseline exists) and the upstream origin
/// (`anchor_url` / `source_revision`) COINCIDE today (no EPUB baseline) but DIVERGE
/// the moment one lands — independent facts, so both are kept. Text/sha invariants
/// live once in [`validate_content_excerpt`]; the URL + revision invariants layer
...
pub struct NormativeExcerpt {
    ...
    /// so the text/sha invariants have a single validator.
    ...
    /// optional, query string allowed. The UPSTREAM origin, distinct from
    /// `excerpt.anchor` (which becomes an EPUB CFI once a baseline exists).
...
// the map edge-GUARD side-table: a place-access condition on an adjacency edge,
...
// `magic-item`). This is a SHAPE CHANGE to an existing field (rename + retype),
// not an additive one, so it DOES need a migration arm: a v37 store may carry
// `"parent": "thing"` on a kind, which the retyped struct would silently DROP on
// read (EntityKind has no `deny_unknown_fields`) — data loss. `load` therefore
// runs `migrate_entity_kind_parent_to_parents` on the raw JSON when the on-disk
// `schema_version < 38`, rewriting each `"parent": "x"` to `"parents": ["x"]`
...
// (missing field `excerpt`). `load` runs `migrate_normative_excerpt_to_wrapped`
// on the raw JSON when the on-disk `schema_version < 42`, reshaping each
// `normative_excerpt` and synthesizing `excerpt.anchor` from the Section's
// `epub_locator` (if present) ELSE from `anchor_url` (`Locator::Prefix(text)`)
// via the SAME `normative_excerpt_anchor` the constructor uses — no field
...
/// non-object `entity_kinds` value or a missing map is a no-op (the typed parse
/// reports any real corruption).
fn migrate_entity_kind_parent_to_parents(raw: &mut serde_json::Value) {
    let Some(kinds) = raw
        .get_mut("entity_kinds")
    ...
    for kind in kinds.values_mut() {
        ...
        let Some(parent) = obj.remove("parent") else {
            continue;
        };
        if obj.contains_key("parents") {
            continue; // already the new shape — the stray legacy key is dropped
        }
        if let serde_json::Value::String(s) = parent {
            if !s.trim().is_empty() {
                obj.insert(
                    "parents".to_string(),
...
/// Round 759 (v41→v42 migration): reshape each Section's flat `normative_excerpt`
/// (`{text, anchor_url, source_revision, text_sha256}`) into the wrapped form
/// (`{excerpt: {anchor, text, text_sha256}, anchor_url, source_revision}`) in the
/// raw JSON, run by `load` BEFORE the typed parse (the retyped struct has no serde
/// default for the new `excerpt` sub-object, so an un-migrated v41 store fails to
/// parse). `excerpt.anchor` is synthesized via [`normative_excerpt_anchor`] — the
/// SAME helper [`build_normative_excerpt`] uses, so a freshly-built excerpt and a
/// migrated one are identical for equal inputs (a fixture parity test pins it).
/// Total + blocker-free: text + hash + the upstream origin are carried verbatim,
...
fn migrate_normative_excerpt_to_wrapped(raw: &mut serde_json::Value) {
    ...
    for section in sections.values_mut() {
        ...
        // Seed the anchor from the Section's EPUB pointer when it has been mirrored
        // (the same precedence build_normative_excerpt applies on a refresh).
        let epub_locator: Option<EpubLocator> = section_obj
            .get("epub_locator")
            .and_then(|v| serde_json::from_value(v.clone()).ok());
        let Some(ne) = section_obj
            .get("normative_excerpt")
            .and_then(serde_json::Value::as_object)
        else {
            continue;
        };
        // Already the new shape (has `excerpt`) — idempotent skip.
        if ne.contains_key("excerpt") {
            continue;
        }
        ...
        let anchor_url = str_field("anchor_url");
        let source_revision = str_field("source_revision");
        let text_sha256 = str_field("text_sha256");
        let anchor = serde_json::to_value(normative_excerpt_anchor(
            &anchor_url,
            &text,
            epub_locator.as_ref(),
        ))
        .expect("ContentAnchor serializes infallibly");
        ...
        section_obj.insert("normative_excerpt".to_string(), wrapped);
    }
}
...
impl AtomicStore {
    ...
    pub fn load(path: &Path) -> Result<Self, AtomicStoreError> {
        ...
        // `schema_version` field's serde default uses (`default_schema_version`),
        // or the pre-v4 audit→publishable clone below silently stops running for
        // it (the two defaults must not drift — the SSOT rule one read-path over).
        ...
        if on_disk_version > CURRENT_SCHEMA_VERSION {
            return Err(AtomicStoreError::SchemaVersionMismatch {
                store: on_disk_version,
                expected: CURRENT_SCHEMA_VERSION,
            });
        }
        // v37→v38 (Round 738): EntityKind.parent (Option<String>) → parents
        // (BTreeSet<String>). Rewrite the raw JSON before the typed parse would
        ...
        // v41→v42 (Round 759): NormativeExcerpt flat → wrapped around
        // ContentExcerpt. Rewrite the raw JSON before the typed parse, which
...
fn build_candidate_section(
    ...
    let excerpt =
        match normative_excerpt {
            // A section being created carries no epub_locator yet (imported later
            // via import_epub_anchors), so the anchor is synthesized from anchor_url.
            Some((text, anchor_url, source_revision, text_sha256)) => Some(
                build_normative_excerpt(text, anchor_url, source_revision, text_sha256, None)?,
            ),
            None => None,
        };
...
/// (R759, P3c): the STABLE external A2 bulk-import contract, decoupled from the
/// nested storage [`NormativeExcerpt`]. The consumer supplies the UPSTREAM origin
/// (`anchor_url`, `source_revision`) + the projected `text` (+ optional extractor
/// `text_sha256`); [`build_normative_excerpt`] synthesizes the content-SSOT anchor.
/// Field-identical to the pre-R759 flat `NormativeExcerpt`, so existing manifests
...
pub struct SectionImport {
    ...
    #[serde(default)]
    pub normative_excerpt: Option<NormativeExcerptImport>,
...
/// The single [`ContentExcerpt`] invariant every write path enforces (R759, P3c;
/// CLAUDE.md multi-write-path / R305 rule). The narrative `import_content_excerpts`
/// / `import_scene_cast` paths and the spec [`build_normative_excerpt`] constructor
/// all route their built excerpt through this before it is stored, so no path can
/// persist a blank anchor source, blank text, or a `text` that does not hash to its
/// declared `text_sha256`. Trailing-newline trimming and the `text_sha256` VALUE
/// (store-computed for narrative, externally-supplied for spec) are per-path input
/// policy — legitimately different, like the polymorphic `anchor.source`; this is
/// the shared post-build check. A populated hash that no longer matches
/// (`Some(false)`) is the only integrity failure; an empty hash is unrevalidatable,
/// not invalid.
fn validate_content_excerpt(excerpt: &ContentExcerpt) -> Result<(), AtomicMutateError> {
    if excerpt.anchor.source.trim().is_empty() {
        return Err(AtomicMutateError::Validation(
            "content excerpt anchor source blank or whitespace-only".to_string(),
        ));
    }
    if excerpt.text.trim().is_empty() {
        return Err(AtomicMutateError::Validation(
            "content excerpt text blank or whitespace-only".to_string(),
        ));
    }
    if excerpt.text_sha256_matches() == Some(false) {
        return Err(AtomicMutateError::Validation(format!(
            "content excerpt text_sha256 mismatch: declared `{}` != sha256(text) `{}` — the cached text does not match its pinned hash",
            excerpt.text_sha256,
            excerpt.recompute_text_sha256()
        )));
    }
    Ok(())
}

/// Synthesize the content-SSOT anchor for a spec [`NormativeExcerpt`]'s `excerpt`
/// (R759, P3c). The EPUB-forward-compatible position: when the Section has been
/// EPUB-mirrored (`epub_locator` present) the anchor points into the EPUB spine
/// (`source = spine_href`; `locator = Cfi` when a CFI is known, else the
/// verbatim-text `Prefix` within that doc); otherwise it falls back to the upstream
/// `anchor_url` with a verbatim-text `Prefix` — the SAME Prefix model narrative
/// uses, so NO new `Locator` variant. `stored_text` is the already-trimmed
/// projected text. This is the ONE synthesis site both the constructor and the
/// v41→v42 migration share (a fixture parity test pins them equal), so they cannot
/// drift. `epub_locator` stays a distinct section-level pointer — synthesis READS
/// it here but does not consume it.
fn normative_excerpt_anchor(
    anchor_url: &str,
    stored_text: &str,
    epub_locator: Option<&EpubLocator>,
) -> mnemosyne_core::ContentAnchor {
    match epub_locator {
        Some(loc) => mnemosyne_core::ContentAnchor {
            source: loc.spine_href.clone(),
            locator: match &loc.cfi {
                Some(cfi) => mnemosyne_core::Locator::Cfi(cfi.clone()),
                None => mnemosyne_core::Locator::Prefix(stored_text.to_string()),
            },
        },
        None => mnemosyne_core::ContentAnchor {
            source: anchor_url.to_string(),
            locator: mnemosyne_core::Locator::Prefix(stored_text.to_string()),
        },
    }
}

/// Validate the spec-excerpt inputs and build the wrapped [`NormativeExcerpt`].
///
/// The single constructor for a `NormativeExcerpt` (R759, P3c). It layers the
/// spec-only invariants (`anchor_url` an absolute http(s) URL with a host,
/// `source_revision` non-blank) then routes the built `excerpt` through the shared
/// [`validate_content_excerpt`] so the text/sha invariant is enforced in ONE place
/// across spec + narrative (CLAUDE.md half-enforced-invariant rule; the R305
/// paste-error class). Both spec write paths route through it — the bulk
/// [`import_sections`] inline-at-create path and the EPUB re-projection path
/// [`import_epub_excerpts`]. `epub_locator` (the Section's, when it has one) seeds
/// the content-SSOT anchor via [`normative_excerpt_anchor`].
///
/// Validates:
/// - `source_revision` non-empty (trimmed).
/// - `anchor_url` parses as absolute URL (scheme `http`/`https` + host).
/// - (shared) `text` non-empty (trimmed) and `text_sha256`, when non-empty, equals
///   `sha256(stored text)` — the stored text is the projected cache and the hash is
///   its revalidation anchor (R403). Empty `text_sha256` = unrevalidatable
///   (hand-authored or pre-v8, surfaced by `report-excerpt-hash-backfill`).
fn build_normative_excerpt(
    ...
    let excerpt = ContentExcerpt {
        anchor: normative_excerpt_anchor(anchor_url, &stored_text, epub_locator),
        text: stored_text,
        text_sha256: hash.to_string(),
    };
    validate_content_excerpt(&excerpt)?;
    Ok(NormativeExcerpt {
        excerpt,
...
pub fn import_content_excerpts(
    ...
    // (R759) — the SAME check the spec build_normative_excerpt path enforces.
    let mut candidates: Vec<(String, ContentExcerpt)> = Vec::with_capacity(excerpts.len());
    for e in excerpts {
        let excerpt = ContentExcerpt {
        ...
        validate_content_excerpt(&excerpt).map_err(|err| {
            AtomicMutateError::Validation(format!(
                "import_content_excerpts: {}: {err}",
                e.section_id
            ))
        })?;
...
pub fn import_scene_cast(
    ...
    // routes through the shared validate_content_excerpt (R759); `entity` is the
    // scene-specific extra.
    for p in presences {
        if p.entity.trim().is_empty() {
            return Err(AtomicMutateError::Validation(format!(
                "import_scene_cast: {} has an empty entity",
                p.section_id
            )));
        }
        let excerpt = ContentExcerpt {
        ...
        validate_content_excerpt(&excerpt).map_err(|err| {
            AtomicMutateError::Validation(format!("import_scene_cast: {}: {err}", p.section_id))
        })?;
    }
...
/// ordering this manifest already relies on). Each `parents` entry (Round 732
/// DEBT-M as one parent, Round 738 as a SET — multiple inheritance) must appear
...
pub struct EntityKindImport {
    ...
    /// omitted / empty ⇒ a root kind (a flat registry, backward-compat).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
///
/// `parents` (Round 732 DEBT-M as one parent, Round 738 as a SET — the DAG /
/// multiple-inheritance extension) are this kind's direct super-kinds. Each
/// (trimmed, non-empty; duplicates and blanks dropped) must ALREADY be a
/// registered kind (the `add_entity` kind-first symmetry — parent-declared-first)
/// and none may be `kind_id` itself (a 1-node self-cycle). Because every parent
/// must pre-exist and there is no parent-mutation setter on this write path, a
/// multi-node cycle is unreachable via it (each new kind is a leaf
/// pointing only at strictly-earlier kinds); the scan detector
/// (`entity_kind_parent_violations`) is defense-in-depth for a hand-edited store
/// or the parent-mutation setter.
pub fn add_entity_kind(
    ...
    // Trim + drop empties + dedup into the stored SET. A new kind is a leaf
    // (nothing references it yet), so naming already-registered parents can
...
/// endpoint-kind constraint under the Round 738 (DEBT-M, generalised to a DAG)
/// inheritance graph: the kind IS `required`, or `required` is reachable by
/// walking UP through the kind's `parents` closure (a `weapon` satisfies a
/// `thing`-scoped rule because `thing ∈ ancestors(weapon)`; a `magic-sword`
/// with parents `{weapon, magic-item}` satisfies BOTH a `weapon`-scoped and a
/// `magic-item`-scoped rule). `None` or an empty kind satisfies nothing — an
/// unspecified kind is a subkind of nothing, preserving Round 701's
/// reject-on-unspecified.
///
...
/// invariant (the R295/R699 lesson). Cycle-safe AND diamond-safe: the `visited`
/// set both terminates a hand-edited parent loop (returns false, does not hang;
/// the loop itself is a separate [`entity_kind_parent_violations`] finding) and
...
pub fn is_kind_or_subkind(store: &AtomicStore, kind: Option<&str>, required: &str) -> bool {
    ...
    // DFS up through every parent; `required` reachable ⇒ subkind. `visited`
    // guards both cycles (terminate, do not hang) and diamonds (expand once).
...
/// Every entity-kind whose `parents` links (Round 732 DEBT-M as a tree, Round
/// 738 as a DAG) break the inheritance-graph invariant — a parent that is NOT
/// in the registry (a dangling ref) OR a `parents` graph that CYCLES. The write
/// path (`add_entity_kind`) forbids a dangling or self parent, and a new leaf
/// cannot close a multi-node cycle; only a future parent-mutation setter
/// or an out-of-band edit can — re-checking at the scan boundary is the
...
///
/// Cycle detection is an iterative three-colour DFS over the parent DAG, MEMOIZED
/// (`black` = proven fully-explored): each kind is entered at most once, so the
/// whole scan is O(V+E), not O(V²) (R732 review LOW-1, generalised from the
/// single-parent chain to the multi-parent graph). A kind is a cycle violation
/// iff its upward closure contains a cycle (equivalently, its parent chains never
/// all bottom out at a root) — so a tail leading INTO a cycle is named too, not
/// only the cycle members. A self-loop (a 1-cycle) is included. Diamonds are NOT
/// cycles: a super-kind reached by two disjoint paths is fully explored once
/// (`black`) and never re-flagged.
pub fn entity_kind_parent_violations(store: &AtomicStore) -> Vec<(String, String)> {
    ...
    // (2) Cycle: iterative three-colour DFS. `black` = fully explored (memoised
    // across roots so each node is entered once). `bad` = its upward closure
    // reaches a cycle. `on_path` is reset per root (the GRAY set); a parent link
    // to an on-path node is a back edge ⇒ that node is on a cycle, and this node
    // (which reaches it) is bad. Badness then flows down the DFS on exit: a node
    // is bad if any parent is bad. A dangling parent is a dead end (skipped —
    // reported in pass 1, cannot be part of a cycle).
    let mut black: BTreeSet<&str> = BTreeSet::new();
    let mut bad: BTreeSet<&str> = BTreeSet::new();
    for root in store.entity_kinds.keys() {
        if black.contains(root.as_str()) {
            continue;
        }
        let mut on_path: BTreeSet<&str> = BTreeSet::new();
        // (node, exiting?) — false = enter, true = post-order exit.
            ...
            if let Some(ek) = store.entity_kinds.get(node) {
                for parent in &ek.parents {
                    ...
                    if on_path.contains(parent) {
                        // Back edge: `parent` is on the current path ⇒ a cycle.
                        bad.insert(parent);
                        bad.insert(node);
                    } else if !black.contains(parent) {
                        stack.push((parent, false));
                    }
                    // parent already black ⇒ its badness is folded in at `node`'s exit.
                }
            }
        }
    }
    ...
    if store.edge_guards.remove(&edge).is_none() {
        return Err(AtomicMutateError::Validation(format!(
            "remove_edge_guard: no edge guard on fact `{edge}` to remove (a guard is added by \
             add_edge_guard and cascade-dropped by retract_fact)"
        )));
    }
    save_with_receipt(
        store,
...
///
/// Round 627 — BUT clearing is not neutral, and R626 shipped this unsaid: the
/// fact falls back to the plan's `default_mode`, which defaults to `Withhold`
/// (and IS `withhold` in all three plans of the real consumer store). So
/// clearing a `state` override flips that fact from told to never-told for that
/// telling. The receipt names the resulting effective mode rather than leaving
/// the author to infer it — `report-disclosure-coverage` would surface it only
/// if they thought to look, and R626's own prescribed workflow (clear, then
/// retract) leaves the fact silently withheld if they stop after step 1.
...
pub fn apply_facts_manifest(
    ...
    for (idx, k) in manifest.entity_kinds.iter().enumerate() {
        ...
        // Round 732 (DEBT-M) / Round 738 (DAG): every parent must be registered
...
/// telling whose plan carries an override keyed by this fact. Read-only; the
/// scan [`retract_fact`] and [`amend_fact`] consult so they stop reaching states
/// [`apply_disclosure_override`] forbids at write time ("the fact exists", and
/// "a withhold/first_at target is typed").
///
/// The write authorities over that relationship are FOUR — `set_disclosure` and
/// `import_facts` (both routed through `apply_disclosure_override`, the only
/// enforcer), plus `retract_fact` and `amend_fact`, which could delete the fact
/// or drop its typed leg from the other side. One invariant, four write paths,
/// one enforcer = the CLAUDE.md half-enforced-invariant anti-pattern. R508 leans
/// on exactly this guarantee in prose ("the targeted facts are guaranteed typed
/// by the R507 set_disclosure invariant") and only re-checks it defensively in
/// the gate; the guarantee was not true.
///
/// Round 627 — this closes the FACT-shaped referent only. A decision also
/// references SECTIONS (`first_at` coords, `surface.scene`) which
/// `apply_disclosure_override` validates at write time and `remove_section`
/// orphans silently — the same asymmetry, not disclosure-specific (it orphans a
/// fact's own `canon_from`/`evidence` identically), so it is its own round.
...
pub fn retract_fact(
    ...
    // silent.
    //
    // Round 627 — the two classes carry SEPARATE remedies, because R626 shipped
    // one sentence of advice for both and it was impossible for half of them: a
    // disclosure referrer is a TELLING, and `retract-fact --fact <telling>`
    // answers "not present in atomic store". A guard whose remedy cannot be
    // followed is the trap R626 was written to avoid, one layer up — so each
    // class names the primitive that actually clears it.
    let fact_referrers = inbound_fact_refs(&store.narrative_facts, id)
        .iter()
    ...
    {
        let mut clauses = Vec::new();
        if !fact_referrers.is_empty() {
            clauses.push(format!(
                "is referenced by {} — retract or amend those facts first",
                fact_referrers.join(", ")
            ));
        }
        if !disclosure_referrers.is_empty() {
            clauses.push(format!(
                "carries a disclosure decision under telling {} — clear each with \
                 `remove_disclosure` (CLI: remove-disclosure --telling <id> --fact {id} \
                 --reason <text>) first",
                disclosure_referrers.join(", ")
            ));
        }
        ...
        return Err(AtomicMutateError::Validation(format!(
            "retract_fact: fact `{id}` {} (a recorded assertion never dangles)",
            clauses.join("; and it ")
        )));
    }
    ...
    // Round 720 — CASCADE-DROP the edge GUARD keyed by this fact (the EDGE side):
    // a guard is subordinate edge metadata, so it goes WITH the edge. The CONDITION
...
mod tests {
 ...
 "schema_version": 3
    ...
    /// Round 738 review (Finding 1) — a store with NO `schema_version` key must
    /// clone audit→publishable on load, exactly as an explicit v1/v3 store does:
    /// the version-less default (`default_schema_version` = 1) is `< 4`. The R738
    /// load refactor read the version from raw JSON, so this default must match
    /// the typed field's serde default or the clone silently stops (the two
    /// read-defaults must not drift). NON-VACUITY: with the pre-fix `unwrap_or(4)`
    /// the version-less store defaulted to 4, `4 < 4` is false, and this assert
    /// went red (publishable stayed None).
    #[test]
    fn version_less_store_clones_audit_into_publishable_on_load() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join(".atomic/workspace.atomic.json");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        // No `schema_version` key at all — the pre-history / hand-authored case.
        let version_less_json = r#"{
 "changelog_entries": {
 "Round 1": {
 "decision_summary": "audit-only summary",
 "changes_bullets": ["audit change"]
 }
 }
}"#;
        std::fs::write(&path, version_less_json).unwrap();
        let loaded = AtomicStore::load(&path).unwrap();
        assert_eq!(
            loaded.schema_version, 1,
            "version-less ⇒ the serde default 1"
        );
        let entry = loaded
            .changelog_entries
            .get("Round 1")
            .expect("Round 1 entry present");
        assert_eq!(
            entry.publishable_decision_summary.as_deref(),
            Some("audit-only summary"),
            "a version-less (pre-v4) store must clone audit→publishable on load"
        );
        assert!(
            entry.publishable_matches_audit(),
            "publishable must match audit after the pre-v4 clone"
        );
    }
 ...
 "schema_version": 4
    ...
    fn every_remover_pairs_with_an_inbound_scan() {
        ...
        let acknowledged: std::collections::BTreeSet<&str> = [
            ...
            "edge_guard", // R720: a side-table entry keyed by an edge fact; removing
            // it strands no ref (it IS a referrer of its condition, not
            // a referenced identity). The inverse hazards — a fact
    ...
    fn seed_excerpt(
        ...
        e.normative_excerpt = Some(NormativeExcerptImport {
            text: text.to_string(),
    ...
    fn build_normative_excerpt_accepts_valid_empty_hash() {
        ...
        .unwrap();
        assert_eq!(ne.excerpt.text, "The <event> element ...");
        ...
        assert_eq!(ne.excerpt.text_sha256, "");
        // With no epub_locator the content-SSOT anchor falls back to anchor_url.
        assert_eq!(
            ne.excerpt.anchor.source,
            "https://www.w3.org/TR/scxml/#event"
        );
    }
    ...
    fn build_normative_excerpt_rejects_blank_text() {
        let err = build_normative_excerpt(" \n ", "https://example.com/spec", "rev1", "", None)
            .unwrap_err();
        assert!(matches!(err, AtomicMutateError::Validation(_)));
    }
    ...
    fn build_normative_excerpt_rejects_blank_source_revision() {
        let err = build_normative_excerpt("text", "https://example.com/spec", "   ", "", None)
            .unwrap_err();
        assert!(matches!(err, AtomicMutateError::Validation(_)));
    }
    ...
    fn build_normative_excerpt_rejects_non_url_anchor() {
        let err = build_normative_excerpt("text", "not-a-url", "rev1", "", None).unwrap_err();
        match err {
            AtomicMutateError::Validation(msg) => {
    ...
    fn build_normative_excerpt_rejects_missing_host() {
        let err =
            build_normative_excerpt("text", "https:///path-only", "rev1", "", None).unwrap_err();
        assert!(matches!(err, AtomicMutateError::Validation(_)));
    }
    ...
    fn build_normative_excerpt_accepts_matching_hash() {
        ...
        let ne =
            build_normative_excerpt(text, "https://example.com/spec", "rev1", &hash, None).unwrap();
        assert_eq!(ne.excerpt.text_sha256, hash);
    }
    ...
    fn build_normative_excerpt_hashes_stored_text_not_raw() {
        ...
        .unwrap();
        assert_eq!(ne.excerpt.text, stored);
        assert_eq!(ne.excerpt.text_sha256, hash);
    }
    ...
    #[test]
    fn build_normative_excerpt_prefers_epub_locator_anchor() {
        // When the Section is EPUB-mirrored the content-SSOT anchor points into the
        // spine (source = spine_href, Cfi when known), not the upstream anchor_url.
        let loc = EpubLocator {
        ...
        .unwrap();
        assert_eq!(ne.excerpt.anchor.source, "OEBPS/spec.xhtml");
        assert_eq!(
            ne.excerpt.anchor.locator,
            mnemosyne_core::Locator::Cfi("epubcfi(/6/4!/4)".to_string())
        );
        // The upstream origin is still kept, distinct from the content-SSOT anchor.
        assert_eq!(ne.anchor_url, "https://www.w3.org/TR/scxml/#event");
    }

    #[test]
    fn content_excerpt_text_sha_three_states() {
        // The text/sha three-state logic now lives on the shared ContentExcerpt
        // substrate (R759); NormativeExcerpt delegates to `.excerpt`.
        let mk = |text: &str, hash: &str| ContentExcerpt {
            anchor: mnemosyne_core::ContentAnchor {
                source: "https://example.com/s".to_string(),
                locator: mnemosyne_core::Locator::Prefix(text.to_string()),
            },
            text: text.to_string(),
            text_sha256: hash.to_string(),
        };
    ...
    #[test]
    fn validate_content_excerpt_enforces_the_shared_invariant() {
        // The single home of the text/anchor/sha invariant (R759): both write paths
        // route through it. Blank anchor source, blank text, or a populated hash that
        // no longer matches are rejected; an empty hash is unrevalidatable (accepted).
        let anchor = |src: &str| mnemosyne_core::ContentAnchor {
            source: src.to_string(),
            locator: mnemosyne_core::Locator::Prefix("x".to_string()),
        };
        let ex = |src: &str, text: &str, sha: &str| ContentExcerpt {
            anchor: anchor(src),
            text: text.to_string(),
            text_sha256: sha.to_string(),
        };
        assert!(validate_content_excerpt(&ex("  ", "prose", "")).is_err());
        assert!(validate_content_excerpt(&ex("src", " \n ", "")).is_err());
        assert!(validate_content_excerpt(&ex("src", "prose", "deadbeef")).is_err());
        assert!(validate_content_excerpt(&ex("src", "prose", "")).is_ok());
        let good = sha256_hex(b"prose");
        assert!(validate_content_excerpt(&ex("src", "prose", &good)).is_ok());
    }

    #[test]
    fn content_excerpt_invariant_parity_across_write_paths() {
        // CLAUDE.md multi-write-path rule (R305): the ContentExcerpt invariant must
        // accept/reject identically whether reached via the spec constructor or the
        // narrative import. Blank text is the shared edge both paths can express
        // (narrative computes its own sha, so a mismatch is structurally impossible
        // there — the spec path owns that edge, asserted above).
        let url = "https://example.com/spec";
        let spec_ok = |text: &str| build_normative_excerpt(text, url, "rev1", "", None).is_ok();
        let narrative_ok = |text: &str| {
            let tmp = TempDir::new().unwrap();
            let path = tmp.path().join(".atomic/workspace.atomic.json");
            let mut store = AtomicStore::new();
            seed_section(&mut store, "s");
            import_content_excerpts(
                &mut store,
                &path,
                &[ContentExcerptImport {
        ...
        // blank text: BOTH reject.
        assert!(!spec_ok(" \n "), "spec path must reject blank text");
        assert!(
            !narrative_ok("   "),
            "narrative path must reject blank text"
        );
        // valid text: BOTH accept.
        assert!(spec_ok("real prose"), "spec path accepts valid text");
        assert!(
            narrative_ok("real prose"),
            "narrative path accepts valid text"
        );
    }

    /// Write a v41-shaped store (flat `normative_excerpt`) to `path` by building a
    /// real v42 store through the API then downgrading its JSON on disk — so every
    /// AtomicSection field is present/valid and only the excerpt shape is legacy.
    fn write_flat_normative_store(
        path: &Path,
        section_id: &str,
        text: &str,
        url: &str,
        ...
        let mut store = AtomicStore::new();
        let mut e = imp(section_id, "docs/GENERATED.md", "T");
        e.normative_excerpt = Some(NormativeExcerptImport {
            text: text.to_string(),
            anchor_url: url.to_string(),
            source_revision: rev.to_string(),
        ...
        raw["schema_version"] = serde_json::json!(41);
        let ne = raw["sections"][section_id]["normative_excerpt"].clone();
        raw["sections"][section_id]["normative_excerpt"] = serde_json::json!({
            "text": ne["excerpt"]["text"],
            "anchor_url": ne["anchor_url"],
            "source_revision": ne["source_revision"],
            "text_sha256": ne["excerpt"]["text_sha256"],
        });
        std::fs::write(path, serde_json::to_vec_pretty(&raw).unwrap()).unwrap();
    }

    #[test]
    fn migrate_flat_normative_excerpt_anchor_url_only() {
        let text = "verbatim spec prose";
        let url = "https://www.w3.org/TR/scxml/#event";
        let sha = sha256_hex(text.as_bytes());
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join(".atomic/workspace.atomic.json");
        write_flat_normative_store(&path, "scxml-3.13", text, url, "2015-09-01", &sha, None);
        let store = AtomicStore::load(&path).unwrap();
        let ne = store
            .section("scxml-3.13")
            .unwrap()
            ...
            .as_ref()
            .unwrap();
        assert_eq!(ne.excerpt.text, text);
        assert_eq!(ne.excerpt.text_sha256, sha);
        assert_eq!(ne.anchor_url, url);
        assert_eq!(ne.source_revision, "2015-09-01");
        // Anchor synthesized from anchor_url, byte-equal to the constructor's.
        let built = build_normative_excerpt(text, url, "2015-09-01", &sha, None).unwrap();
        assert_eq!(ne.excerpt.anchor, built.excerpt.anchor);
        assert_eq!(ne.excerpt.anchor.source, url);
        assert_eq!(
            ne.excerpt.anchor.locator,
            mnemosyne_core::Locator::Prefix(text.to_string())
        );
    }

    #[test]
    fn migrate_flat_normative_excerpt_prefers_epub_locator() {
        let text = "verbatim spec prose";
        let url = "https://www.w3.org/TR/scxml/#event";
        let sha = sha256_hex(text.as_bytes());
        let loc = EpubLocator {
        ...
        let ne = store
            ...
            .as_ref()
            .unwrap();
        // Anchor seeded from the EPUB pointer, equal to the constructor's with loc.
        let built = build_normative_excerpt(text, url, "rev1", &sha, Some(&loc)).unwrap();
        assert_eq!(ne.excerpt.anchor, built.excerpt.anchor);
        assert_eq!(ne.excerpt.anchor.source, "OEBPS/spec.xhtml");
        assert_eq!(
            ne.excerpt.anchor.locator,
        ...
        // epub_locator itself is preserved (a distinct section-level pointer).
        assert_eq!(
            store.section("scxml-3.13").unwrap().epub_locator.as_ref(),
            Some(&loc)
        );
        // Idempotent: saving (→v42) then reloading leaves the wrapped shape intact.
        store.save(&path).unwrap();
        let reloaded = AtomicStore::load(&path).unwrap();
        assert_eq!(
            reloaded.section("scxml-3.13").unwrap().normative_excerpt,
            store.section("scxml-3.13").unwrap().normative_excerpt
        );
        assert_eq!(reloaded.schema_version, CURRENT_SCHEMA_VERSION);
    }

    #[test]
    fn migrate_flat_normative_excerpt_edge_url() {
        // A query-string / no-fragment anchor_url still migrates (Prefix over the URL).
        let text = "edge prose";
        let url = "http://host.example/path?q=1&r=2";
        let sha = sha256_hex(text.as_bytes());
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join(".atomic/workspace.atomic.json");
        write_flat_normative_store(&path, "edge", text, url, "rev1", &sha, None);
        let store = AtomicStore::load(&path).unwrap();
        let ne = store
            .section("edge")
            .unwrap()
    ...
    fn import_epub_excerpts_overwrites_existing_hash() {
        ...
        let ne = store
            ...
            .as_ref()
            .unwrap();
        assert_eq!(ne.excerpt.text, "second");
        assert_eq!(ne.excerpt.text_sha256, h2);
    }
    ...
    fn import_sections_inline_excerpt_anchored_at_create() {
        ...
        let ne = store
            ...
            .as_ref()
            .unwrap();
        assert_eq!(ne.excerpt.text, "verbatim spec");
    ...
    fn import_sections_rejects_invalid_excerpt() {
        ...
        assert!(format!("{err}").contains("text blank"), "{err}");
    }
    ...
    fn disclosure_refs_hold_across_retract_and_amend() {
        ...
        for fact in ["f-withheld", "f-craft"] {
            ...
            // Round 627 — assert the REMEDY, not just the label. R626 pinned only
            // "via disclosure override" + "never dangles" and so locked in advice
            // ("retract or amend the referrers first") that is impossible for this
            // class: the referrer is a TELLING. A refusal must name the primitive
        ...
        //
        // Round 627 — the WRITE half of this control was a vacuous assertion in
        // R626: it named a fact that was never added, so it passed on "fact not
        // present" and asserted an ERROR under a comment claiming acceptance —
        // leaving the accept leg of an R305 parity pin untested, and R626's
        // ledger citing it as evidence it never produced. Both halves are real
        // now: the write path ACCEPTS this input, and so must the amend path.
        set_disc(&mut store, &path, "t", "f-craft", "state", &[], None).unwrap();
        add_fact(&mut store, &path, &untyped_import("f-untyped-ok")).unwrap();
        set_disc(&mut store, &path, "t", "f-untyped-ok", "state", &[], None)
    ...
    /// Round 720 — `add_edge_guard` gates BOTH facts exist (edge + condition, a
    /// dangling-ref check) and rejects a self-guard; the value is the condition
    /// fact id. NON-VACUITY: each rejection asserts its loud message.
    #[test]
    fn add_edge_guard_gates_both_facts_and_self() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("s.json");
        let mut store = AtomicStore::new();
        seed_chapters(&mut store);
        ...
        let err = add_edge_guard(&mut store, &path, "f-gone", "f-key")
            ...
            .to_string();
        assert!(
            err.contains("edge fact") && err.contains("not present"),
            "{err}"
        );
        // Missing condition (dangling-ref) rejects.
        let err = add_edge_guard(&mut store, &path, "f-edge", "f-nokey")
            .unwrap_err()
            .to_string();
        assert!(
            err.contains("condition fact") && err.contains("not present"),
            "{err}"
        );
        // Self-guard rejects.
        let err = add_edge_guard(&mut store, &path, "f-edge", "f-edge")
            .unwrap_err()
            .to_string();
        assert!(err.contains("cannot guard itself"), "{err}");
    }

    /// Round 720 — the DELETE rule, BOTH sides: retract the EDGE fact cascade-drops
    /// its guard (metadata follows the edge); retract the CONDITION fact is REFUSED
    /// while a guard references it (the R707-block peer, NEW machinery since the
    /// ref is in the side-table); `remove_edge_guard` is the escape hatch and the
    /// out-of-band orphan cleaner. NON-VACUITY: the refuse names the referring edge.
    #[test]
    fn edge_guard_delete_cascade_and_condition_refuse() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("s.json");
        let mut store = AtomicStore::new();
        seed_chapters(&mut store);
        ...
        // Retracting the CONDITION is refused while the guard references it.
        let err = retract_fact(&mut store, &path, "f-key", "test")
            .unwrap_err()
            .to_string();
        assert!(
            err.contains("guard CONDITION") && err.contains("f-edge"),
            "the referring edge is named: {err}"
        );
        ...
        add_fact(&mut store, &path, &sample_fact("f-key2", "gt")).unwrap();
    ...
    /// Round 738 (DAG) — the ONE resolver `is_kind_or_subkind`: a kind IS its
    /// `required` OR `required` is reachable by walking UP through the kind's
    /// `parents` closure (directional — a PARENT is not a subkind of its child);
    /// None / empty resolve to nothing; a hand-edited cycle TERMINATES (false,
    /// no hang); 0 parent links ⇒ exact-match (backward-compat). MULTIPLE
    /// INHERITANCE: a kind with two parents satisfies rules scoped to EITHER; a
    /// DIAMOND (one ancestor reached by two paths) resolves once, no hang.
    #[test]
    fn is_kind_or_subkind_resolves_the_ancestor_closure() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("s.json");
        let mut store = AtomicStore::new();
        // thing ⊃ weapon ⊃ blade ; potion is a sibling subtree of thing.
        add_entity_kind(&mut store, &path, "thing", &[], "").unwrap();
        ...
        add_entity_kind(&mut store, &path, "magic-item", &[], "").unwrap();
        add_entity_kind(
            &mut store,
            &path,
            "magic-sword",
        ...
        add_entity_kind(&mut store, &path, "thing2", &[], "").unwrap();
    ...
    /// write path (`add_entity_kind`: self-parent + unregistered parent reject,
    /// for EVERY parent) and the scan boundary (`entity_kind_parent_violations`
    /// via `store_registry_violations`: an out-of-band dangling parent OR cycle).
    /// A MULTI-parent write with all parents registered is clean; naming any one
    /// bad parent rejects. NON-VACUITY: a clean DAG passes both.
    #[test]
    fn entity_kind_parent_write_path_and_scan_parity() {
        ...
        assert!(
            store_registry_violations(&store).is_empty(),
            "a clean kind DAG passes the scan"
        );

        // WRITE PATH rejects a self-parent and an unregistered parent — and
        // rejects when ONE of several parents is bad (all-or-nothing).
        let err = add_entity_kind(&mut store, &path, "loop", &["loop"], "").unwrap_err();
        assert!(err.to_string().contains("its own parent"), "{err}");
        let err = add_entity_kind(&mut store, &path, "orphan", &["ghost"], "").unwrap_err();
    ...
    /// clean (the O(V+E) three-colour DFS does not false-flag a long chain), a
    /// DIAMOND is NOT a cycle (a super-kind reached by two paths is fine), a TAIL
    /// leading INTO a cycle names every node whose closure never bottoms out at a
    /// root, and a SELF-LOOP (a 1-cycle) is flagged.
    #[test]
    fn entity_kind_parent_detector_deep_chain_diamond_and_cycles() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("s.json");
        let mut store = AtomicStore::new();
        // A 200-deep legitimate chain via the mutate API: k0 root, k{i}⊂k{i-1}.
        add_entity_kind(&mut store, &path, "k0", &[], "").unwrap();
        for i in 1..200 {
            let parent = format!("k{}", i - 1);
            add_entity_kind(&mut store, &path, &format!("k{i}"), &[parent.as_str()], "").unwrap();
        }
        // A DIAMOND built through the write path: d ⊂ {l, r}, both ⊂ top.
        add_entity_kind(&mut store, &path, "top", &[], "").unwrap();
        add_entity_kind(&mut store, &path, "l", &["top"], "").unwrap();
        add_entity_kind(&mut store, &path, "r", &["top"], "").unwrap();
        add_entity_kind(&mut store, &path, "d", &["l", "r"], "").unwrap();
        assert!(
            entity_kind_parent_violations(&store).is_empty(),
            "a deep acyclic chain + a diamond must validate clean"
        );
        ...
        for k in ["a", "b", "t", "z"] {
            assert!(
                ...
                "`{k}` (in or leading into a cycle) must be flagged: {v:?}"
            );
        }
        // The deep clean chain and the diamond are untouched by the injected cycles.
        assert!(
            !v.iter()
                .any(|(id, _)| id.starts_with('k') || ["top", "l", "r", "d"].contains(&id.as_str())),
            "the acyclic chain + diamond must stay clean: {v:?}"
        );
    }
