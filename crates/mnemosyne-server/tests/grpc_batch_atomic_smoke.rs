...
async fn atomic_batch_with_one_reject_blocks_all_storage_writes() {
 ...
 assert_eq!(collected.len(), 3, "atomic batch must emit one result per proposal");
 for r in &collected {
 assert!(
 !r.accepted,
...
async fn atomic_batch_with_all_accepts_commits_all_in_one_writebatch() {
 ...
 for r in &collected {
 assert!(r.accepted, "all-accept atomic batch must accept every proposal; rejected {}", r.proposal_id);
...
async fn non_atomic_batch_preserves_per_proposal_98_semantics() {
 ...
 assert_eq!(collected.len(), 3);
 assert!(collected[0].accepted, "non-atomic: valid proposal 1 must accept");
 assert!(!collected[1].accepted, "non-atomic: orphan proposal must reject");
 assert!(collected[2].accepted, "non-atomic: valid proposal 3 must accept (surrounding orphan)");

 // Storage: 100 and 102 committed, 101 not (orphan didn't write).
 assert!(store_for_check.get(CfId::Entities, 1, 100, 100000).expect("get 100").is_some());
 assert!(store_for_check.get(CfId::Entities, 1, 102, 102000).expect("get 102").is_some());
