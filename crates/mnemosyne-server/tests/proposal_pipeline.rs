...
use mnemosyne_server::{
 AuditAppender, MnemosyneServer, MnemosyneService, Proposal, ProposalKind,
};
...
fn cross_ref_with_zero_target_rejected_with_audit_record() {
 ...
 let r = server.submit(&p).unwrap();
 assert!(!r.accepted);
 assert!(r.rejection_reason.unwrap().contains("to_section unresolved"));
 let audit = server
 .handler()
 .audit()
 .read(r.audit_transaction_id.unwrap())
