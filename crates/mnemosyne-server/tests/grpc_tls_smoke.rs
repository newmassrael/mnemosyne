...
async fn grpc_tls_handshake_round_trips_submit_proposal() {
 ...
 assert!(response.accepted, "TLS path must accept the well-formed proposal");
 assert!(response.audit_transaction_id_set, "audit_transaction_id flag must be set on accept");
 assert_eq!(response.audit_transaction_id, 1, "first audit txn on a fresh store");
