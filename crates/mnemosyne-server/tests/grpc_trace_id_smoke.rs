...
async fn traceparent_header_propagates_into_audit_record_trace_id() {
 ...
 let response = client.submit_proposal(req).await.expect("submit").into_inner();
...
async fn missing_traceparent_falls_back_to_server_minted_uuid() {
 ...
 let resp1 = client
 .submit_proposal(encode_proposal(entity_create_proposal("p-trace-fallback-1")))
 .await
 .expect("submit 1")
 .into_inner();
 let resp2 = client
 .submit_proposal(encode_proposal(entity_create_proposal("p-trace-fallback-2")))
 .await
 .expect("submit 2")
 .into_inner();
 ...
 assert_ne!(t1, t2, "two requests without traceparent must mint distinct ids");
...
async fn pre_round_99_audit_record_decodes_with_trace_id_none() {
 ...
 store.put(CfId::Audit, 0, 0, 42, &bytes).expect("legacy put");
