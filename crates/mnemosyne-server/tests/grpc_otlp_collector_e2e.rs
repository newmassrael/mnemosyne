...
const SAMPLE_TRACEPARENT: &str =
 "00-cafef00dcafef00dcafef00dcafef00d-cafef00dcafef00d-01";
...
async fn end_to_end_otlp_collector_receives_handler_emitted_spans() {
 ...
 let guard = init_otlp_tracing_subscriber(&collector_endpoint)
 ...
 let resp = client.submit_proposal(req).await.expect("submit").into_inner();
