...
use crate::audit::{
 append_outcome_with_trace_context, AuditAppender, AuditFanout, TraceContext,
};
use crate::error::Result;
use crate::gate::{
 DefaultTier1, DefaultTier2, DefaultTier3, GateOutcome, GateTier, Tier1Gate, Tier2Gate, Tier3Gate,
};
...
impl ProposalHandler {
 ...
 let audit = AuditAppender::with_broadcast_capacity_and_fanout(
 Arc::clone(&store),
 capacity,
 fanout,
 );
 ...
 let outcome = match (t1, t2) {
  ...
  (GateOutcome::Accept { .. }, GateOutcome::Accept { .. }) => {
  GateOutcome::Accept { warnings: warnings.clone() }
  }
 };
 ...
 let composite_reason = format!(
  "atomic batch rejected: {batch_reject_reason}"
 );
 ...
 let txn = tracing::info_span!("audit.append", outcome = "atomic_accept")
  .in_scope(|| {
  append_outcome_with_trace_context(&self.audit, p, &outcome, ctx)
  })?;
 results.push(ProposalResult {
  proposal_id: p.proposal_id.clone(),
  accepted: true,
 ...
 ProposalKind::CrossRefCreate {
  ...
  self.store
  .put(CfId::Relations, *branch_id, *from_section, *to_section, value)?;
 }
...
fn proposal_storage_tuple(
 proposal: &Proposal,
) -> Option<(CfId, u64, u64, u64, Vec<u8>)> {
 match &proposal.kind {
 ProposalKind::EntityCreate {
 branch_id,
 entity_id,
