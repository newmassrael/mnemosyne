...
impl Mnemosyne for MnemosyneGrpcService {
    async fn submit_proposal(
        ...
        let proposal = decode_proposal(wire).map_err(Status::invalid_argument)?;
        ...
        tokio::spawn(async move {
            loop {
                ...
                let proposal = match decode_proposal(wire) {
                    ...
                    Err(reason) => {
                        if tx
                            .send(Err(Status::invalid_argument(reason)))
                            .await
                            .is_err()
                        {
                            return;
    ...
    async fn subscribe_audit_trail(
        ...
        let span =
            tracing::info_span!("subscribe_audit_trail", grpc.method = tracing::field::Empty,);
        let _enter = span.enter();
        ...
        let matches_filters = std::sync::Arc::new(move |record: &AuditRecord| -> bool {
        ...
        tokio::spawn(async move {
            ...
            let history = tokio::task::spawn_blocking(
                move || -> std::result::Result<(usize, u64), String> {
                    ...
                    let scan_result =
                        scan_handler
                            .audit()
                            .iter_from_streaming(from_txn, |record| {
                ...
                loop {
                    ...
                    match next {
                        Ok(record) => {
                            ...
                            let txn = record.transaction_id;
                            if tx.send(Ok(encode_audit_record(&record))).await.is_err() {
                                return;
                            }
                            emitted += 1;
...
pub fn build_reflection_service() -> Result<
...
pub fn build_rustls_server_config(
    ...
    let mut cfg = if let Some(ca) = client_ca_pem {
        ...
        for c in ca_certs {
            roots.add(c).map_err(|e| format!("client ca add: {e}"))?;
        }
...
impl TlsIdentityRotator {
    ...
    pub fn new(initial: std::sync::Arc<rustls::ServerConfig>) -> (Self, TlsIdentityHandle) {
    ...
    pub fn rotate(&self, new_config: std::sync::Arc<rustls::ServerConfig>) -> Result<(), String> {
        self.sender
            .send(new_config)
...
pub fn decode_result(wire: proto::ProposalResult) -> ProposalResult {
    ProposalResult {
        ...
        audit_transaction_id: wire
            .audit_transaction_id_set
            .then_some(wire.audit_transaction_id),
