...
impl AuditAppender {
    ...
    pub fn append_accepted(&self, proposal: &Proposal, warnings: &[String]) -> Result<u64> {
    ...
    fn write(&self, record: &AuditRecord) -> Result<()> {
        ...
        let payload = serde_json::to_vec(record)
            .map_err(|e| ServerError::AuditViolation(format!("audit serde: {e}")))?;
        self.store
    ...
    pub fn read(&self, transaction_id: u64) -> Result<Option<AuditRecord>> {
        ...
        match bytes {
            None => Ok(None),
            Some(b) => {
                let r: AuditRecord = serde_json::from_slice(&b)
                    .map_err(|e| ServerError::AuditViolation(format!("audit decode: {e}")))?;
                Ok(Some(r))
            }
        }
    }
        ...
        F: FnMut(AuditRecord) -> bool,
    {
        ...
        self.store
            .iter_branch_entity_streaming(CfId::Audit, 0, 0, |txn_id, bytes| {
                if txn_id < start {
                    return true;
                }
...
mod tests {
    ...
    async fn slow_subscriber_with_full_broadcast_returns_lagged() {
        ...
        match first {
            ...
            other => {
                panic!("first recv after slow-subscriber overflow must be Lagged, got {other:?}")
            }
        }
        ...
        let next = rx
            .try_recv()
            .expect("recv after Lagged must surface a record");
        assert!(
