//! Phase -1A stage 2D sled criterion harness.
//!
//! Mirrors direct-impl's `branching_sla.rs` so the cross-backend comparison
//! reads off the same SLA criteria with the same sample / measurement-time
//! settings (sample 1000+, jitter < 30%).
...
use sled_baseline::{populate_full, synthesize_linear_chain, BranchMeta, BranchStore};
