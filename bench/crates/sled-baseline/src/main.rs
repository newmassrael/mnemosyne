...
#[command(
    name = "sled-bench",
    version,
    about = "Phase -1A stage 2D sled sanity check"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
fn smoke(
    ...
    println!(
        ...
        stats.branches,
        stats.root_entities,
        stats.overlay_writes,
        stats.asset_rows,
        stats.asset_ref_rows,
        pop_elapsed
    );
...
fn cold_point_query(
    ...
    print_hist(
        &format!(
            "cold_point_query_depth_{:02}_cache_{}MiB",
            chain_depth, cache_mib
        ),
        &hist,
        target,
    );
...
fn storage_growth(
    ...
    println!(
        "sled API size : {} bytes ({:.2} MiB)",
        api_size,
        api_size as f64 / 1024.0 / 1024.0
    );
