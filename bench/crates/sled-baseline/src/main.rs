//! Phase -1A stage 2D sled sanity-check CLI (DESIGN.md §18 line 1851).
//!
//! Mirrors the direct-impl CLI subcommand surface so cross-backend
//! measurements use the same fixture and the same reporting shape.
...
#[derive(Parser, Debug)]
#[command(name = "sled-bench", version, about = "Phase -1A stage 2D sled sanity check")]
struct Cli {
 #[command(subcommand)]
 cmd: Cmd,
}
...
enum Cmd {
 /// End-to-end smoke run: populate workload + each branching SLA scenario.
 Smoke {
 #[arg(long)]
 ...
 /// Cache-cold point query at a given chain depth.
 ColdPointQuery {
 ...
 /// 200K asset storage growth measurement.
 StorageGrowth {
 #[arg(long)]
 workload: Option<PathBuf>,
 #[arg(long)]
...
fn main() -> Result<()> {
 ...
 let cli = Cli::parse();
 match cli.cmd {
 Cmd::Smoke { workload, overlay_pct, iters, cache_mib } => {
 smoke(workload.as_deref(), overlay_pct, iters, cache_mib)
 }
 Cmd::ColdPointQuery { chain_depth, iters, cache_mib, workload } => {
 cold_point_query(chain_depth, iters, cache_mib, workload.as_deref())
 }
 Cmd::Throughput { writers, hot_set, duration_secs, cache_mib } => {
 throughput(writers, hot_set, duration_secs, cache_mib)
 }
 Cmd::StorageGrowth { workload, out, overlay_pct, cache_mib } => {
 storage_growth(workload.as_deref(), &out, overlay_pct, cache_mib)
 }
 }
}
...
fn smoke(
 ...
 println!("sled path: {}", dir.path().display());
...
fn cold_point_query(
 ...
 let workload = load_or_generate(workload_path)?;
 let dir = TempDir::new()?;
 println!("sled path: {} cache: {} MiB", dir.path().display(), cache_mib);
 let store = BranchStore::open(dir.path(), cache_mib)?;
...
fn throughput(
 ...
 println!("=== Phase -1A stage 2D §12 reframed throughput (sled) ===");
 println!(
 ...
 let dir = TempDir::new()?;
 println!("sled path: {}", dir.path().display());
 let store = ConcurrentStore::open(dir.path(), cache_mib)?;
 ...
 println!(
 "§12 reframe: backend-comparable headline = commits/s (sled CAS conflicts substitute for OCC abort)"
 );
 println!("\nper-writer:");
...
fn storage_growth(
 ...
 let store = BranchStore::open(out, cache_mib)?;
 let t0 = Instant::now();
 let stats = populate_full(&store, &workload, overlay_pct)?;
 ...
 store.flush()?;
 let api_size = store.size_on_disk()?;
 drop(store);
 ...
 println!("\n--- §18 line 1944 storage growth (sled) ---");
 println!("apparent size : {}", du_text);
 println!("on-disk size : {}", du_real_text);
 println!("sled API size : {} bytes ({:.2} MiB)", api_size, api_size as f64 / 1024.0 / 1024.0);
