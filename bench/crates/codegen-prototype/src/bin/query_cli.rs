...
fn main() -> Result<()> {
    ...
    if args.json && args.include_related && args.include_changelog {
        let envelope = build_envelope(&workspace, &section_id)
    ...
    if args.json {
        let view = section_by_id(&workspace, &section_id)
            .ok_or_else(|| anyhow!("section_id `{}` workspace in not found", section_id))?;
...
fn parse_args(args: Vec<String>) -> Result<CliArgs> {
    ...
    for arg in args {
        match arg.as_str() {
            ...
            other => {
                if let Some(already) = &out.section_id {
                    bail!("section_id argument duplicate (already `{already}`)");
                }
