...
pub fn emit_closure_runtime(rules: &[ClosureRule]) -> EmittedClosure {
    ...
    out.push_str(
        "// Round 55 decision ratify carry — closure_runtime module's mnemosyne itself codegen\n",
    );
    ...
    out.push_str(
        "// ─── Closure aggregator — bounded forward-chaining (DESIGN §11 default k=2) ─\n",
    );
    ...
    out.push_str(&format!(
        "pub const MAX_CLOSURE_DEPTH: u32 = {};\n\n",
        max_depth
    ));
...
fn emit_rule_expand_fn(out: &mut String, rule: &ClosureRule) {
    ...
    for (i, p) in rule.premises.iter().enumerate() {
        let role_str: Vec<String> = p.roles.iter().map(|(r, v)| format!("{r}={v}")).collect();
        out.push_str(&format!(
            "/// ({i}) {} {{ {} }}\n",
            p.predicate,
            role_str.join(", ")
...
fn emit_closure_metadata_cpp(rules: &[ClosureRule]) -> String {
    ...
    let total_predicates: usize = rules.iter().map(|r| r.premises.len() + 1).sum();
    out.push_str(&format!(
    ...
    for f1 in &snapshot {
        ...
        for f2 in &snapshot {
            ...
            for (role, var) in &rule.premises[0].roles {
                if let Some((_, val)) = f1.roles.iter().find(|(r, _)| r == role) {
            ...
            for (role, var) in &rule.premises[1].roles {
                if let Some((_, val)) = f2.roles.iter().find(|(r, _)| r == role) {
                    if let Some(existing) = binding.get(var) {
                        if existing != val {
                            bind_ok = false;
                            break;
...
mod tests {
    ...
    fn emit_per_rule_expand_function() {
        ...
        assert!(emitted
            .source
            .contains("pub fn expand_transitive_grandparent"));
        assert!(emitted
            .source
            .contains("pub fn expand_knowledge_modus_ponens"));
    ...
    fn emit_documentation_includes_rule_doc() {
        ...
        assert!(emitted
            .source
            .contains("Rule `transitive_grandparent` (depth 2)"));
        assert!(emitted
            .source
            .contains("Rule `knowledge_modus_ponens` (depth 1)"));
