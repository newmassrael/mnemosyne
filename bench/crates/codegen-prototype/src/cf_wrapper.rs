...
pub fn emit_wrapper(spec: &GraphSpec, layout: &CfLayout) -> EmittedWrapper {
    ...
    out.push_str(
        "use rocksdb::{ColumnFamilyDescriptor, DB, IteratorMode, Direction, WriteBatch};\n",
    );
...
fn emit_entity_wrapper(out: &mut String, entity: &EntityDef, meta: &CfMeta) -> usize {
    ...
    out.push_str(
        " pub fn write_batch(&self, entries: &[(u64, u64, u64, Vec<u8>)]) -> Result<()> {\n",
    );
    out.push_str(&format!(
        " let cf = self.db.cf_handle({}).ok_or_else(|| anyhow!(\"missing CF {{}}\", {}))?;\n",
        cf_const, cf_const
...
fn emit_relation_wrapper(out: &mut String, relation: &RelationDef, meta: &CfMeta) -> usize {
    ...
    out.push_str(
        " pub fn iter_from(&self, branch_id: u64, from_id: u64) -> Result<Vec<(u64, Vec<u8>)>> {\n",
    );
    out.push_str(&format!(
        " let cf = self.db.cf_handle({}).ok_or_else(|| anyhow!(\"missing CF {{}}\", {}))?;\n",
        cf_const, cf_const
...
fn emit_migration_stub(out: &mut String, layout: &CfLayout) -> usize {
    ...
    out.push_str(
        " /// CF schema_version per CF — bumped when the schema changes; otherwise stable.\n",
    );
...
pub fn emit_cpp_readonly(spec: &GraphSpec, layout: &CfLayout) -> String {
    let mut out = String::new();
    out.push_str(
        "// Auto-generated by codegen-prototype (Round 53, §42 cf_wrapper C++ readonly emit).\n",
    );
    out.push_str(
        "// Round 36 paradigm carry — 5-language emit contract's *partial break* audit trail\n",
    );
    out.push_str(
        "// (Rust store layer + C++ runtime SDK only emit, Kotlin/Python/Protobuf not.mit).\n",
    );
    out.push('\n');
...
mod tests {
    ...
    fn design_doc_fixture_emits_all_wrappers() {
        ...
        assert!(emitted
            .source
            .contains("pub fn iter_branch(&self, branch_id: u64"));
        assert!(emitted
            .source
            .contains("pub fn iter_from(&self, branch_id: u64"));
    ...
    fn emitted_key_encoding_matches_entity_indexer() {
        ...
        assert!(emitted
            .source
            .contains("BigEndian::write_u64(&mut buf[0..8], branch_id)"));
        assert!(emitted
            .source
            .contains("BigEndian::write_u64(&mut buf[8..16], entity_id)"));
        assert!(emitted
            .source
            .contains("BigEndian::write_u64(&mut buf[16..24], valid_from)"));
    ...
    fn cf_descriptors_emit_lists_all_cfs() {
        ...
        assert!(emitted
            .source
            .contains("pub fn cf_descriptors() -> Vec<ColumnFamilyDescriptor>"));
        assert!(emitted
            .source
            .contains("ColumnFamilyDescriptor::new(CF_SECTION_ENTITIES"));
        assert!(emitted
            .source
            .contains("ColumnFamilyDescriptor::new(CF_CROSS_REF_RELATIONS"));
    }
    ...
    fn migration_stub_emits_versions_and_scaffolding() {
        ...
        assert!(emitted
            .source
            .contains("pub fn scaffold_new_cf(name: &str)"));
    ...
    fn fixture_emit_size_stable() {
        ...
        assert_eq!(emitted.method_count, 3 * 4 + 3 + 3 + 1);
