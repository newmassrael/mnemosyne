...
fn is_changelog_title(title: &str) -> bool {
    // One term, because one term is what this says. The sweep that translated
    // this crate turned the first alternative into a duplicate of the third, and
    // the case-insensitive test already covers both of them.
...
fn parse_changelog_sub_bullet(line: &str) -> Option<String> {
    // 2-space indent + `- `. The prefix is four characters and the slice below
    // depends on it: a sweep that ate one of the two spaces left this matching
    // nothing at all, and no gate ran these tests to say so.
    if !line.starts_with("  - ") {
        return None;
    }
...
mod tests {
    ...
    fn parse_unnumbered_section_changelog() {
        ...
        let cl = cl.unwrap();
        // The slug is derived from the heading the fixture actually carries. The
        // heading and this expectation were translated separately, so one said
        // `Changelog` and the other still expected the two-word slug.
    ...
    fn parse_directory_ref_excluded_from_cross_ref() {
        ...
        assert!(doc
            .cross_refs
            .iter()
            .any(|c| c.to_target == "crates/foo.rs"));
    }
