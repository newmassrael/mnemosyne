...
use codegen_prototype::closure_runtime::{
    closure_small_fixture, emit_closure_runtime, run_closure_in_memory, InMemoryFact, ProvenanceTag,
};
...
use codegen_prototype::salsa_wire::{
    cascade_metadata_canonical_set, design_doc_cascade_fixture,
    emit_cascade_metadata_all_languages, emit_salsa_wire,
};
...
fn main() {
    ...
    println!(
        " - emitted source hash (DefaultHasher): 0x{:016x}",
        source_hash
    );
    println!();
    ...
    println!();
    println!(
        "[measure 4] KEY_LEN constant: {} (expected 24, DESIGN §18 line 1845 carry)",
        KEY_LEN
    );
    println!();
    ...
    println!(
        " - secondary-readable subset count: {}",
        wrapper.secondary_cf_count
    );
    println!();
    ...
    println!(
        "[measure 4] direct-impl signature consistency (manual BranchStore ↔ codegen wrapper):"
    );
    println!(" - codegen emit methods: {:?}", sig_check.emitted_methods);
    println!(
    ...
    println!(
        " - #[salsa::tracked] emit: {}",
        if has_tracked { "PASS" } else { "FAIL" }
    );
    println!(
        " - CascadeDb trait emit: {}",
        if has_db_trait { "PASS" } else { "FAIL" }
    );
    println!(
        " - dependency graph metadata emit: {}",
        if has_dep_graph { "PASS" } else { "FAIL" }
    );
    println!(
        " - CascadeOrdering axis emit: {}",
        if has_ordering { "PASS" } else { "FAIL" }
    );
    println!();
 ...
 cf_partial.partial_languages
    ...
    let det_cf =
        sha256_hex(&cf_partial2.rust) == h_cf_rust && sha256_hex(&cf_partial2.cpp) == h_cf_cpp;
    println!(
 ...
 if cf_pass { "PASS" } else { "FAIL" }
    ...
    println!(
        "source-of-truth: bench/crates/codegen-prototype/src/cf_wrapper.rs (emit_cpp_readonly)"
    );
 ...
 cm.edge_count, cm.query_count
    ...
    let runtime_excluded = ["kotlin", "python", "cpp", "protobuf"]
        ...
        .all(|(_, e)| {
            !e.contains("#[salsa::input]")
                && !e.contains("#[salsa::tracked]")
                && !e.contains("CascadeDb")
        });
    println!(
 ...
 if runtime_excluded { "PASS" } else { "FAIL" }
    ...
    let derived_count = facts
        .iter()
        .filter(|f| f.provenance == ProvenanceTag::Derived)
        .count();
    ...
    println!(
        " - no Kotlin (data class): {}",
        if no_kotlin { "PASS" } else { "FAIL" }
    );
    println!(
        " - no Python (@dataclass): {}",
        if no_python { "PASS" } else { "FAIL" }
    );
    println!(
        " - no C++ (namespace mnemosyne): {}",
        if no_cpp_namespace { "PASS" } else { "FAIL" }
    );
    println!(
        " - no Protobuf (message): {}",
        if no_proto { "PASS" } else { "FAIL" }
    );
    println!(
        " - Rust-only marker comment: {}",
        if rust_marker { "PASS" } else { "FAIL" }
    );
    println!(
 ...
 "FAIL"
    ...
    let decision_refs = parsed
        ...
        .count();
    println!(
        "[measure 4] typed facts shape (DESIGN §61 mapping table 13 row's subset validation):"
    );
 ...
 " → DESIGN §61 *Phase 0 implementation entry prerequisite framing* (Round 61)'s *small fixture validation feasibility validation source* carry"
    ...
    println!(
        " - input ParsedDoc — sections={} / changelog_entries={} / cross_refs={}",
        parsed.sections.len(),
 ...
 "FAIL"
    ...
    println!(
        ...
        if diff.changelog_sequence_match {
            "MATCH"
        } else {
            "DIFF"
        }
