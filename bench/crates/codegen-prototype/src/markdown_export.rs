...
pub fn emit_markdown(doc: &ParsedDoc) -> String {
    ...
    for section in &doc.sections {
        ...
        for cr in doc
            .cross_refs
            .iter()
            .filter(|c| c.from_section == section.section_id)
        {
            if !first {
    ...
    if !doc.changelog_entries.is_empty() {
        ...
        for entry in &doc.changelog_entries {
            ...
            for sub in &entry.sub_bullets {
                // Two spaces: the import side only recognises a sub-bullet at that
                // indent, so emitting one space breaks the round trip this crate exists
                // to measure.
                out.push_str(&format!("  - {}\n", sub));
            }
        }
...
fn build_section_depth_map(doc: &ParsedDoc) -> std::collections::BTreeMap<&str, usize> {
...
mod tests {
    ...
    fn anchor_preserves_cjk() {
        // The input has to CARRY CJK or this asserts nothing about the pass-through
        // it is named for: a sweep replaced the heading with an ASCII one and
        // translated the expectations word by word, leaving a test that could not
        // hold and nothing that ran it.
        let s = to_github_anchor("변경 이력");
        assert!(s.contains("변경"));
        assert!(s.contains("이력"));
        assert!(s.contains('-'));
    }
