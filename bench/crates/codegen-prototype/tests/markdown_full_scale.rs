...
fn full_scale_parse_summary_dump() {
 ...
 "Round-trip integrity dimensionper PASS rate (DESIGN §61 *preserved mandatory dimension* 3 dimension):"
    ...
    println!(" - cross_ref set match: {}/{}", cr_pass, DOC_PATHS.len());
    println!(
...
fn design_md_parse_recognizes_section_id_39_56_61_66() {
    ...
    assert!(
        section_ids.contains("39"),
        "§39 section_id recognize failure"
    );
    assert!(
        section_ids.contains("56"),
        "§56 section_id recognize failure"
    );
    assert!(
        section_ids.contains("61"),
        "§61 section_id recognize failure"
    );
    assert!(
        section_ids.contains("66"),
        "§66 section_id recognize failure"
    );
}
...
fn dump_round_trip_diff_per_file() {
    ...
    for path in DOC_PATHS {
        ...
        if !diff.section_identity_match {
            ...
            println!(
                " only-in-original ({} total, first 8):",
                a_keys.difference(&b_keys).count()
            );
            for k in &only_a {
                println!(" {:?}", k);
            }
            println!(
                " only-in-reparsed ({} total, first 8):",
                b_keys.difference(&a_keys).count()
            );
            for k in &only_b {
                println!(" {:?}", k);
            }
        ...
        if !diff.cross_ref_set_match {
            ...
            println!(
                " cross_ref only-in-original ({} total, first 8):",
            ...
            println!(
                " cross_ref only-in-reparsed ({} total, first 8):",
...
fn dump_design_md_orphan_detail() {
        ...
        } = err
        {
            ...
            for (lno, line) in lines.iter().enumerate() {
                ...
                while let Some(pos) = line[search_from..].find(&needle_bare) {
                    ...
                    let is_boundary = !matches!(next_char, Some(c) if c.is_ascii_digit());
            ...
            println!(
                " total raw occurrences of §{} (boundary-checked): {}",
                to_target, hit_count
            );
            ...
            let known_ids: std::collections::BTreeSet<&str> = parsed
                ...
                .collect();
            println!(
                " to_target ∈ section_id_set: {}",
                known_ids.contains(to_target.as_str())
            );
            // dump from_section title + parent_section for navigation context.
            if let Some(sec) = parsed
                .sections
                .iter()
                .find(|s| s.section_id == *from_section)
            {
                println!(
    ...
    println!("source-of-truth: bench/crates/codegen-prototype/tests/markdown_full_scale.rs");
    println!(
        "classification (real fragile / parser bug / cross-doc intent) → Round 69 design ratify"
    );
}
...
fn dump_remaining_orphans_detail() {
  ...
  default-doc lookup (Round 70 OPTION H-2) handles this in production.",
        ...
        let _ = from_section;
        let looks_numeric_section = to_target.chars().all(|c| c.is_ascii_digit() || c == '.');
