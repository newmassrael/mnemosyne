...
#[command(
    name = "lmdb-bench",
    version,
    about = "Phase -1A stage 2D LMDB sanity check"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
fn smoke(workload_path: Option<&std::path::Path>, overlay_pct: f64, iters: usize) -> Result<()> {
...
fn storage_growth(
    ...
    println!(
        "apparent size : {} (sparse — LMDB pre-allocates the map)",
        du_text
    );
    println!(
        "on-disk size : {} (actual blocks, this is the comparator)",
        du_real_text
    );
    println!(
        "data.mdb size : {} bytes ({:.2} MiB)",
        api_size,
