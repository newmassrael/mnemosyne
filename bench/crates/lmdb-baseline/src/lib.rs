...
impl BranchStore {
    ...
    pub fn point_query(
        ...
        while let Some(b) = current {
            let upper = CompositeKey {
                branch_id: b,
                entity_id,
                valid_from: time,
            }
            .encode();
            let lower = CompositeKey {
                branch_id: b,
                entity_id,
                valid_from: 0,
            }
...
pub fn populate_root(store: &BranchStore, workload: &Workload) -> Result<()> {
...

    {
        ...
        store
            .branch_meta
            .put(&mut wtxn, &key, &bincode::serialize(&root_meta)?)?;
    }
    for b in &workload.branches {
        ...
        store
            .branch_meta
            .put(&mut wtxn, &key, &bincode::serialize(&meta)?)?;
    }
...
pub fn populate_assets(store: &BranchStore, workload: &Workload) -> Result<(usize, usize)> {
    ...
    for a in &workload.assets {
        ...
        store
            .assets
            .put(&mut wtxn, &key, &bincode::serialize(&row)?)?;
        asset_count += 1;
...
mod tests {
    ...
    fn flatten_materializes_overlay() {
        ...
        store
            ...
            .unwrap();
        store
            .flatten_branch(leaf, &[7], 100, 9000, "flat".into())
            .unwrap();
