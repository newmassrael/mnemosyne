...
pub mod concurrent;
pub use concurrent::{run_abort_rate, AbortRateReport, ConcurrentStore, WriterStats};
...
impl CompositeKey {
    pub fn encode(self) -> [u8; KEY_LEN] {
        ...
        BigEndian::write_u64(
            &mut buf[ENTITY_ID_OFFSET..VALID_FROM_OFFSET],
            self.entity_id,
        );
...
impl BranchStore {
    ...
    pub fn point_query(
        ...
        while let Some(b) = current {
            ...
            let mut iter = self
                .db
                .iterator_cf(entities_cf, IteratorMode::From(&upper, Direction::Reverse));
