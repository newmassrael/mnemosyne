//! Phase -1A stage 2B branching SLA criterion harness.
//!
//! §18 line 1831-1839 (six measurement criteria) + §18 line 1906-1922
//! (incompleteness gate: sample 1000+ / jitter < 30%). Each criterion group
//! uses `sample_size ≥ 1000` and `measurement_time ≥ 5s` so HDR percentile
//! estimates clear the floor.

use std::time::Duration;

use criterion::{criterion_group, criterion_main, BenchmarkId, Criterion, Throughput};
use direct_impl::{populate_full, synthesize_linear_chain, BranchMeta, BranchStore};
use rand::Rng;
...
impl Fixture {
    fn build() -> Self {
        ...
        let store = BranchStore::open(dir.path(), 256).expect("open store");
...
fn bench_cross_branch_diff(c: &mut Criterion) {
    ...
    group.sample_size(100); // diff is heavier — keep sample modest.
...
fn bench_save_tree_n100(c: &mut Criterion) {
    ...
    let entities = fixture.entity_ids();
    let sample: Vec<u64> = entities.into_iter().take(500).collect();
    // Build a "save tree" of 100 child branches off a single base branch.
