...
#[command(
    name = "workload-gen",
    version,
    about = "Phase -1A stage 1 synthetic workload generator"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
fn cmd_generate(cfg: &WorkloadConfig, output: &PathBuf) -> Result<()> {
    ...
    println!(
        ...
        report.assets.actual, report.facts.actual, report.branches.actual, gen_elapsed, ser_elapsed
    );
    if !report.all_ok() {
...
fn cmd_describe(path: &PathBuf) -> Result<()> {
    ...
    println!(
        " branches {:>7} target {}",
        r.branches.actual, r.branches.target
    );
    println!(" agents {:>7} target {}", r.agents.actual, r.agents.target);
    println!(
        " entities {:>7} target {}",
        r.entities.actual, r.entities.target
    );
    print_branch_tree_stats(&w);
    print_asset_ref_stats(&w);
    Ok(())
...
fn cmd_verify(path: &PathBuf) -> Result<()> {
    ...
    if regen != w {
        bail!(
            "determinism FAIL — regenerated workload diverges from {}",
            path.display()
        );
    }
