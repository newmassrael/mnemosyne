...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
pub enum Query {
    PointLookup { entity_id: u64, branch_id: u64 },
    ThreeHopGraph { start: u64, branch_id: u64 },
    TemporalRange { entity_id: u64, from: u64, to: u64 },
    CrossBranchDiff { branch_a: u64, branch_b: u64 },
    EpistemicQuery { agent_id: u64, fact_id: u64 },
}
...
mod tests {
    ...
    fn fact_refs_resolve() {
        ...
        let w = generate(&cfg);
        let entity_ids: std::collections::HashSet<u64> = w.entities.iter().map(|e| e.id).collect();
        for f in &w.facts {
        ...
        let branch_ids: std::collections::HashSet<u64> = w.branches.iter().map(|b| b.id).collect();
        for a in &w.assets {
