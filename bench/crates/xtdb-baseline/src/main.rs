...
use xtdb_baseline::{agent_doc, entity_doc, fact_doc, person_doc, XtdbClient, DEFAULT_URL};

#[derive(Parser)]
#[command(
    name = "xtdb-bench",
    version,
    about = "Phase -1A stage 2A XTDB baseline runner"
)]
struct Cli {
...
async fn wait(client: &XtdbClient, timeout_secs: u64) -> Result<()> {
    ...
    bail!(
        "xtdb did not respond within {}s — last error: {}",
        timeout_secs,
        last_err
    );
}
...
async fn bench_remaining(
 ...
 pid
        ...
        if !violations.is_empty() {
            bail!(
                "T2 invariant violated for p{} — should not happen with monotonic seeding",
                pid
            );
        }
    ...
    let edn_k2 =
        "{:find [?d ?gp] :where [[?d :parent ?p] [?p :parent ?gp]] :limit 200000}".to_string();
