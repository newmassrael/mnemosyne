...
- **The CLI is `$MN`, never a PATH copy**: `MN="$(git rev-parse --show-toplevel)/scripts/mn"`
  builds this tree's source on every call, so `report-playable-world` (R557) and
  `report-quest-graph` (R568) are present exactly when the source has them. This bullet
  told the orchestrator to install the binary into `~/.cargo/bin` until Round 963 — a slot
  shared with the consumer checkouts on this machine, where overwriting a sibling's pinned
  build has already happened once (Round 823). Confirm: `"$MN" report-quest-graph
