...
mode is `withhold`, and its `first_at` is pinned to sc-20; its surface is the
scene sc-20 and the entity `e-cistern-lid`, the lid she comes up through. That
single override — in `facts.json`, `disclosure_plans` → telling `told` →
override on `at-yrsa-05` — is what says so.
